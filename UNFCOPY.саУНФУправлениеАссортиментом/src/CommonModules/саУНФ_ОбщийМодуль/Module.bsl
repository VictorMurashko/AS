...
// Гарантия формируется в чеке.
// Условие: Это ЮрЛицо или ФизЛицо с интернет доставкой
// Параметры:
//  Ссылка - ДокументСсылка.ЗаказПокупателя - Ссылка
// 
// Возвращаемое значение:
//  Булево - Гарантия формируется в чеке
Функция ГарантияФормируетсяВЧеке(Ссылка)

	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	саУНФ_НастройкиСЦ.ВидЗаказа
	|ИЗ
	|	РегистрСведений.саУНФ_НастройкиСЦ КАК саУНФ_НастройкиСЦ
	|ГДЕ
	|	НЕ саУНФ_НастройкиСЦ.ВидЗаказа = ЗНАЧЕНИЕ(Справочник.ВидыЗаказовПокупателей.ПустаяСсылка)";
	...
	|	И НЕ ЗаказПокупателя.Контрагент.ВидКонтрагента = &ВидКонтрагента
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	И ЗаказПокупателя.Контрагент.ВидКонтрагента = &ВидКонтрагента";

	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("ВидКонтрагента", Перечисления.ВидыКонтрагентов.ФизическоеЛицо);
...
// Получить список аккумуляторов.
// Выбираем всю номенклатуру с подходящей категорией
// Категории содержаться в регистре НастройкиСЦ
// Параметры:
//  Ссылка - ДокументСсылка.ЗаказПокупателя
// 
// Возвращаемое значение:
//  ТаблицаЗначений - Получить список аккумуляторов
Функция ПолучитьСписокАккумуляторов(Ссылка) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|		ПО ВТ_ДОК.НоменклатураКатегорияНоменклатуры = ВТ_КАТ.КатегорияНоменклатуры";

	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	ТаблицаАккумуляторов = Запрос.Выполнить().Выгрузить();

	Возврат ТаблицаАккумуляторов;
КонецФункции

// Нужна ли гарантия.
// Проверяет есть ли в таблице аккумуляторы и продажа не на РТТ
// Параметры:
//  Ссылка - ДокументСсылка.ЗаказПокупателя
// 
// Возвращаемое значение:
//  Булево - Нужна ли гарантия
Функция НужнаЛиГарантия(Ссылка) Экспорт
	ФормироватьВЧеке=ГарантияФормируетсяВЧеке(Ссылка);
	Если ФормироватьВЧеке Тогда
		Возврат Ложь;
	КонецЕсли;

	ТаблицаАккумуляторов = ПолучитьСписокАккумуляторов(Ссылка);
	Если ТаблицаАккумуляторов.Количество() = 0 Тогда
		Возврат Ложь;
	КонецЕсли;
...
// Заменить контрагента в гарантийных талонах.
// Меняет в гарантийном талоне контрагента
// Параметры:
//  Ссылка - ДокументСсылка.ЗаказПокупателя - Ссылка
Процедура ЗаменитьКонтрагентаВГарантийныхТалонах(Ссылка) Экспорт
	...
	КонецЕсли;
КонецПроцедуры
// Получить гарантийные талоны по документу.
// 
// Параметры:
//  Ссылка - ДокументСсылка.ЗаказПокупателя - Ссылка
// 
// Возвращаемое значение:
//  ТаблицаЗначений - гарантийные талоны по документу
Функция ПолучитьГарантийныеТалоныПоДокументу(Ссылка) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ГарантийныеТалоны.Номенклатура,
	|	ГарантийныеТалоны.Контрагент,
	|	ГарантийныеТалоны.Номер,
	|	ГарантийныеТалоны.СерийныйНомер,
	|   ГарантийныеТалоны.СрокГарантии,
	|   ВЫБОР КОГДА ГарантийныеТалоны.Количество =0 ТОГДА 1 ИНАЧЕ ГарантийныеТалоны.Количество КОНЕЦ КАК Количество
	|ИЗ
	|	РегистрСведений.саУНФГарантийныеТалоны КАК ГарантийныеТалоны
	|ГДЕ
	|	ВЫРАЗИТЬ(ГарантийныеТалоны.Документ КАК Документ.ЗаказПокупателя) = &Ссылка";

	Запрос.УстановитьПараметр("Ссылка", Ссылка);

	ТаблицаГарантийных = Запрос.Выполнить().Выгрузить();
	Возврат ТаблицаГарантийных;
КонецФункции
// Выписанные гарантийные талоны.
// Проверяет полностью ли выписаны гарантийные талоны
// Параметры:
//  Ссылка - ДокументСсылка.ЗаказПокупателя - Ссылка 
// 
// Возвращаемое значение:
//  Булево - Выписанные гарантийные талоны
Функция ВыписанныеГарантийныеТалоны(Ссылка) Экспорт
	ТаблицаАккумуляторов = ПолучитьСписокАккумуляторов(Ссылка);
	ТаблицаАккумуляторов.Свернуть("Номенклатура", "Количество");
	
	ТаблицаГарантийных = ПолучитьГарантийныеТалоныПоДокументу(Ссылка);
	Если ТаблицаГарантийных.Количество()=0 Тогда
		Возврат Ложь;
	КонецЕсли;
	
	ТаблицаГарантийных.Свернуть("Номенклатура","Количество");
	
	Результат = ОбщегоНазначения.КоллекцииИдентичны(ТаблицаАккумуляторов, ТаблицаГарантийных);
	Возврат Результат;
КонецФункции

Функция МассивГарантийныхТалоновНаВыписку(Ссылка) Экспорт
	ТаблицаАккумуляторов = ПолучитьСписокАккумуляторов(Ссылка);
	ТаблицаАккумуляторов.Свернуть("Номенклатура","Количество");
	...
	Результат = ОбщегоНазначения.КоллекцииИдентичны(ТаблицаАккумуляторов, ТаблицаГарантийных);
	МассивКВыписке = Новый Массив();
	Если НЕ Результат Тогда
		МассивАккумуляторов = ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаАккумуляторов);
		МассивГарантийных = ОбщегоНазначения.ТаблицаЗначенийВМассив(ТаблицаГарантийных);
		МассивКВыписке = ОбщегоНазначенияКлиентСервер.РазностьМассивов(МассивАккумуляторов, МассивГарантийных);
	КонецЕсли;

     Возврат МассивКВыписке;
КонецФункции

Функция СписокКатегорий() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		СписокКатегорий.Добавить(Выборка.КатегорияНоменклатуры, , , );
	КонецЦикла;

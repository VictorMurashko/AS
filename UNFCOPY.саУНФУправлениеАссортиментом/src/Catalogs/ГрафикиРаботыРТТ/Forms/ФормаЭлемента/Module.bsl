...
Функция ИмяШаблона(Ш)

	Возврат ВРЕГ(СОКРЛП(Ш.Наименование));
...
Процедура ШаблонЗаполненияПриИзменении(Элемент)
	...
	Если ПустаяСтрока(Объект.Наименование)  Тогда
	          Объект.Наименование = ИмяШаблона(ШаблонЗаполненияГрафика);
	КонецЕсли; 
КонецПроцедуры
...
Процедура ИзменитьДниБезПерерывов(СписокДней, ЗначениеПоля, ЕстьОбработанныеОбласти = Ложь)
	...
	Для Каждого ЭлементСписка из СписокДней Цикл
		...
		//***Начало***21.05.2021***МурашкоВ
		Если (Не ЗначениеЗаполнено(ЗначениеПоля) или ЗначениеПоля = "0") ИЛИ ЗначениеПоля <> "V" Тогда
		//***Конец***21.05.2021***МурашкоВ
		...
		Попытка
			//***Начало***21.05.2021***МурашкоВ
			НовоеЗначениеПоля =ПолучитьЗначениеПоля(ЗначениеПоля);
			...
			Если НовоеЗначениеПоля > 24 Тогда
				...
				ВосстановитьПредыдущиеЗначанияПолей(СписокДней);
				Прервать;
			...
			РасчитатьРабочиеЧасы(НомерМесяца);
...
&НаКлиенте
Процедура ВосстановитьПредыдущиеЗначанияПолей(СписокДней)
	...
	Для Каждого ЭлементСписка из СписокДней Цикл
		...
		ПрошлоеЗначениеПоля = СтрокаДанных["День"+НомерДня];
		//***Начало***21.05.2021***МурашкоВ
		Если ПрошлоеЗначениеПоля >0 Тогда
		        ПрошлоеЗначениеПоля ="V";
			Иначе
				ПрошлоеЗначениеПоля ="";
		КонецЕсли; 
		      ТекущаяОбластьЦикла.Текст = ПрошлоеЗначениеПоля;
		//ТекущаяОбластьЦикла.Текст = Строка(?(ПрошлоеЗначениеПоля = 0,"",ПрошлоеЗначениеПоля));
		//***Конец***21.05.2021***МурашкоВ
...
Процедура ИзменитьДниСУчетомПерерывов(СписокДней, НовоеЗначениеПоля, ЕстьОбработанныеОбласти = Ложь)
	...
	Попытка
		//***Начало***21.05.2021***МурашкоВ
		 НовоеЗначениеПоля =ПолучитьЗначениеПоля(НовоеЗначениеПоля); 
	...
	Если НовоеЗначениеПоляКорректно и Не ИнтервалыПерерывовКорректны(СписокДней, НовоеЗначениеПоля) Тогда
		...
		ОткрытьФорму("Справочник.ГрафикиРаботыРТТ.Форма.ФормаИзменитьРасписание", ПараметрыОткрытия,,,,, ОписаниеОповещения, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
&НаСервере
Функция ПолучитьЗначениеПоля(ЗначениеПоля)
	Если ЗначениеПоля = "V" Тогда
		ЗначениеПоля =  8;
	Иначе
		ЗначениеПоля =  0;
	КонецЕсли; 
...
Процедура ВывестиРасписаниеПоГрафику(НомерГода = Неопределено)
	...
	МакетРасписаниеПоГрафику = Справочники.ГрафикиРаботыРТТ.ПолучитьМакет("ГрафикРаботы");
		...
		Для НомерДня = 1 По 31 Цикл
			...
			Иначе
				...
				Иначе
					...
					КонецЕсли;
					Если СтрокаДанных[КолонкаДень]>0 Тогда
					         Область.Текст = "V";
					Иначе	
					        Область.Текст = "";
					КонецЕсли; 
...
Процедура ЗаполнитьГрафикНаСервере(ИзменениеГодаИлиДатыОтсчета = Ложь)
	...
	Если ИзменениеГодаИлиДатыОтсчета Тогда
		//ГрафикМенеджер = Справочники.ГрафикиРаботы;
		  ГрафикМенеджер = Справочники.ГрафикиРаботыРТТ;
		ГрафикМенеджер.ЗаполнитьДаннымиПроизводстенногоКалендаря(ДанныеГрафика, НомерТекущегоГода,,,,ДатаОтсчетаДляЗаполнения, ТипГрафика, ИзмененныеДни, Объект);
		Возврат
	КонецЕсли;
...
Процедура ЗаполнитьДанныеГрафика(НомерГода = Неопределено)
	...
	|	ГрафикиРаботы.ШаблонЗаполнения = ЗНАЧЕНИЕ(Справочник.ШаблоныЗаполненияГрафиковМагазиновИФилиалов.ПустаяСсылка) КАК Изменен,
	|	НАЧАЛОПЕРИОДА(ГрафикиРаботы.ВремяНачала, ДЕНЬ) КАК ВремяНачала
	|ИЗ
	|	РегистрСведений.ГрафикиРаботыРТТ КАК ГрафикиРаботы
	|ГДЕ
	...
	|	ГрафикиРаботы.ШаблонЗаполнения = ЗНАЧЕНИЕ(Справочник.ШаблоныЗаполненияГрафиковМагазиновИФилиалов.ПустаяСсылка),
...
Процедура ПодготовитьИзмененныеДни(НомерГода = Неопределено)
	...
	|	ГрафикиРаботы.ГрафикРаботы = &ГрафикРаботы
	|	И ГрафикиРаботы.Год = &Год
	|	И ГрафикиРаботы.ШаблонЗаполнения = ЗНАЧЕНИЕ(Справочник.ШаблоныЗаполненияГрафиковМагазиновИФилиалов.ПустаяСсылка)
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерМесяца";
...
Процедура ЗаполнитьПоШаблону()
	...
	Если Не ПроизводственныйКалендарьЗаполнен(Объект.ПроизводственныйКалендарь, НомерТекущегоГода) Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для заполнения по шаблону необходио заполнить производственный календарь на %ГодКалендаря% год.'");
...
Процедура ОбновитьПроизводственныйКалендарьГрафика(ПриОткрытииФормы = Ложь, НомерГода = Неопределено, ЭтоОчисткаГрафика = Ложь)
	...
	Если ЗначениеЗаполнено(Объект.ПроизводственныйКалендарь) Тогда
		...
		Если ПроизводственныйКалендарьЗаполнен(Объект.ПроизводственныйКалендарь, НомерГода) Тогда
			ГрафикМенеджер.ЗаполнитьДаннымиПроизводстенногоКалендаря(ДанныеГрафика, НомерГода, Истина,,ПриОткрытииФормы, ДатаОтсчета, ТипГрафика,,Объект);
		Иначе
			ГрафикМенеджер.ЗаполнитьДаннымиПроизводстенногоКалендаря(ДанныеГрафика, НомерГода, Истина, Истина, ПриОткрытииФормы, ДатаОтсчета, ТипГрафика,,Объект);
...
Процедура ЗафиксироватьИзмененныйПериодОдинДень(Результат, ДопПараметрыОповещения) Экспорт
	...
	Если Не Результат = Неопределено и Результат.Свойство("Периоды") Тогда
		...
		РасчитатьРабочиеЧасы(СтрокаДанных.НомерМесяца);
...
Процедура ЗафиксироватьИзмененныйПериодВыделенныеДни(Результат, ДопПараметрыОповещения) Экспорт
	...
	Если Не Результат = Неопределено и Результат.Свойство("Периоды") Тогда
		...
		Для Каждого ВыделеленныйДень из ДопПараметрыОповещения.СписокДней Цикл
			
			ОчиститьСтрокиИзмененныхДней(ВыделеленныйДень.Значение);
			
			Для Каждого СтрокаПериод из Результат.Периоды Цикл
				
				НоваяСтрока = ИзмененныеДни.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаПериод);
				НоваяСтрока.ДатаДня = ВыделеленныйДень.Значение;
				НоваяСтрока.Длительность = СтрокаПериод.Длительность;
			...
			Если Не ЧасовПоДню = 0 Тогда
				СтрокаДанных = ДанныеГрафика[Месяц(ВыделеленныйДень.Значение)-1];
				
				НомерДня = День(ВыделеленныйДень.Значение);
...
&НаКлиенте
Процедура РасчитатьРабочиеЧасы(НомерМесяца)
...
Процедура ЗаписатьДанныеВРегистр(ГодЗаписи, Отказ = Ложь, ТекущийОбъект)
	...
	НачатьТранзакцию();
	//***Начало***21.05.2021***МурашкоВ
	НаборЗаписей =РегистрыСведений.ГрафикиРаботыРТТ.СоздатьНаборЗаписей();
...
Процедура ИзменитьРасписаниеПериода()
	...
	Если ВыделенныеОбласти.Количество() = 1 
		...
		Исключение
			...
			Сообщение = Новый СообщениеПользователю;
			Сообщение.Текст = "Выделите день или период календаря в таблице!";
		...
		Для ТекСтрока = ТекОбласть.Верх По ТекОбласть.Низ Цикл
			Для ТекКолонка = ТекОбласть.Лево По ТекОбласть.Право Цикл
				Если ТекКолонка >= 3 И ТекКолонка <= 33 И ТекСтрока >= 2 И ТекСтрока <= 13 Тогда
					...
					СписокДней.Добавить(ДатаДня, Строка(НомерДняЦикла), Изменен);
					ЧасвоРаботы = СтрокаДанных["День"+НомерДня];
	...
	ПараметрыОткрытия.Вставить("Изменен", Ложь);
	ПараметрыОткрытия.Вставить("ИзмененныеДни", ИзмененныеДни);
	ПараметрыОткрытия.Вставить("ЧасовРаботы", ЧасвоРаботы);
	ПараметрыОткрытия.Вставить("ВыбранИнтервал", Истина);
	ПараметрыОткрытия.Вставить("СписокДней", СписокДней);

...
Функция СоздатьЗаказыПокупателя(ТаблицаРаспределения2,Организация,ТранзитныйСклад)
	Запрос = Новый Запрос;
	...
	Пока ВыборкаФилиалы.Следующий() Цикл   //ПО ФИЛИАЛАМ
		...
		Пока ВыборкаМагазины.Следующий() Цикл   //ПО МАГАЗИНАМ
			...
			СтруктураОбщих.Магазин = ТранзитныйСклад;
			СписокКонтрагент = Новый Массив(1);
			//СписокКонтрагент[0] = СтруктураОбщих.Магазин;
			СписокКонтрагент[0] = ТранзитныйСклад;
...
Процедура СделатьНепроведеннымиЗаказы(МенеджерВременныхТаблиц,ТранзитныйСклад)
	Запрос = Новый Запрос;
...
Функция НормыПоМАП(МенеджерВременныхТаблиц,Организация,СтруктураНастроек)
	...
	Запрос.УстановитьПараметр("Поставщик",СтруктураНастроек.Поставщик);    
	Запрос.УстановитьПараметр("Организация",Организация);
	Запрос.УстановитьПараметр("МинскОсновной",СтруктураНастроек.СкладМинск); 
...
Запрос.Текст ="ВЫБРАТЬ
              ...
              |	саУНФ_НормыАссортиментаСрезПоследних.МинимальныйЗапас КАК МинимальныйЗапас
              |ПОМЕСТИТЬ ВТ_НОМ
              |ИЗ
              ...
              |	ВТ_НОМ.МинимальныйЗапас КАК МинимальныйЗапас
              ...
              |	ВТ_НОМЦЕНЫ.МинимальныйЗапас КАК МинимальныйЗапас
              |ПОМЕСТИТЬ ВТ_НОРМЫ
              |ИЗ
...
Функция ОстаткиВМагазинах(МенеджерВременныхТаблиц,Организация,ТранзитныйСклад)
	НачатьТранзакцию();
...
Функция ПолучитьНедостатки(МенеджерВременныхТаблиц,Организация,МинскОсновной,ОсновнойСклад)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВТ_НОРМЫ.МинимальныйЗапас КАК МинимальныйЗапас,
	               |	ВТ_НОРМЫ.Цена КАК Цена,
	               |	&ОсновнойСклад КАК Филиал
	               |ПОМЕСТИТЬ ВТ_НОРМЫ2
	               |ИЗ
	               |	ВТ_НОРМЫ КАК ВТ_НОРМЫ
	               |;
	               ...
	               |	ВЫБОР
	               |		КОГДА ЕСТЬNULL(ВТ_ОСТМАГ2.СвободныеОстатки, 0) > ВТ_НОРМЫ2.МинимальныйЗапас
	               |			ТОГДА 0
	               |		ИНАЧЕ ВЫБОР
	...
	//Таб=саУНФ_ОбщийМодуль.ПоказатьВременнуюТаблицу(МенеджерВременныхТаблиц,"ВТ_НОРМЫ2") ;
...
Функция ОстаткиВФилиалах(МенеджерВременныхТаблиц,Организация,МинскОсновной,ОсновнойСклад)
	МассивФилиалов = Новый Массив(1);
	//МассивФилиалов[0]	=	МинскОсновной;
	МассивФилиалов[0]	=	ОсновнойСклад;
...
Процедура РассчитатьПотребности() Экспорт
	СтруктураНастроек = саУНФ_АтозаказВызовСервера.ОбщиеПараметрыАвтозаказа();
	МинскОсновной  = СтруктураНастроек.СкладМинск;
	ОсновнойСклад =  СтруктураНастроек.СкладПинск;
	...
	ЕстьДоставка = НормыПоМАП(МенеджерВременныхТаблиц,Организация,СтруктураНастроек);
	Если НЕ  ЕстьДоставка Тогда
	        Возврат;
	КонецЕсли; 
	
	СделатьНепроведеннымиЗаказы(МенеджерВременныхТаблиц,СтруктураНастроек.ТранзитныйСклад);
	
	ОстаткиВМагазинах(МенеджерВременныхТаблиц,Организация,СтруктураНастроек.ТранзитныйСклад);
	...
	Если ТаблицаРаспределения2.Количество()>0 Тогда
		  СоздатьЗаказыПокупателя(ТаблицаРаспределения2,Организация,СтруктураНастроек.ТранзитныйСклад);
	КонецЕсли; 
	...
	Сообщение.Текст = "Товар готов для отправки в Минск";
	Сообщение.Сообщить();
КонецПроцедуры

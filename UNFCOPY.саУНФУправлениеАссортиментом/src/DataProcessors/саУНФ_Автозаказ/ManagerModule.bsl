...
Функция НормыПоМАП(МенеджерВременныхТаблиц,Организация,Поставщик)
	...
	Запрос.УстановитьПараметр("Поставщик",Поставщик);    
	Запрос.УстановитьПараметр("Организация",Организация); 
	Запрос.УстановитьПараметр("ДатаОбработки",ТекущаяДатаСеанса()+КоличествоСуток*Сутки);
...
Процедура ЗаказМелочевки() Экспорт
	Если НачалоДня(ТекущаяДатаСеанса())=Дата(2022,1,14) Тогда
	     Возврат;
	КонецЕсли; 
	...
	Если ДеньНедели(ТекущаяДатаСеанса())<>СтруктураПараметров.ДеньНедели Тогда
		     Возврат;
	КонецЕсли;	

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УправлениеНебольшойФирмойСервер.НастроитьФормуОбъектаМобильныйКлиент(Элементы, "Заказы");
...
Процедура ПриОткрытии(Отказ)
	...
	Если Не  СобственнаяДоставка Тогда
		Элементы.СтраницаКурьеры.ТекущаяСтраница = Элементы.СтраницаСтороннийСервис;
	Иначе
		Элементы.СтраницаКурьеры.ТекущаяСтраница = Элементы.СтраницаСобственнаяДоставка;
	КонецЕсли; 
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Конец СтандартныеПодсистемы.УправлениеДоступом
	 Для каждого СтрокаТУ  Из ТаблицаУдаленных Цикл
	          ЗаказНаУдаление = СтрокаТУ.Заказ;
			  ЗаказНаУдалениеОбъект =  ЗаказНаУдаление.ПолучитьОбъект();
...
Процедура КурьерПриИзменении(Элемент)
	Объект.ТелефонКурьера = саУНФ_ОбщийМодуль.ТелефонКурьера(Объект.Курьер);
...
&НаСервере
Процедура ЗаказыПередУдалениемНаСервере()
	Для Каждого Строка Из Элементы.Заказы.ВыделенныеСтроки Цикл
		НСтроки = Объект.Заказы.НайтиПоИдентификатору(Строка).НомерСтроки;    
		ЗаказНаУдаление =   Объект.Заказы[НСтроки-1].Заказ;
		Если ЗначениеЗаполнено(ЗаказНаУдаление) Тогда
			     НоваяСтрока= ТаблицаУдаленных.Добавить();
				 НоваяСтрока.Заказ= ЗаказНаУдаление;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗаказыПередУдалением(Элемент, Отказ)
	ЗаказыПередУдалениемНаСервере();
КонецПроцедуры

&НаСервере
Функция ОбнулитьКурьера()
	Если  ВариантДоставки = "СлужбаДоставки" И ЗначениеЗаполнено(Объект.Курьер) Тогда
		Объект.Курьер  = Справочники.ФизическиеЛица.ПустаяСсылка();
	ИначеЕсли ВариантДоставки = "Курьер"	Тогда
		Объект.ФИОВодителя   = Объект.Курьер.Наименование; 
	КонецЕсли; 

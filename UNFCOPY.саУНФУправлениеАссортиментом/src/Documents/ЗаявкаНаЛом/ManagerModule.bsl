...
Процедура ПроверитьВозможностьВводаНаОснованииЗаказаПокупателя(ДанныеЗаполнения, ЗначенияРеквизитов) Экспорт
	...
	Если ЗначенияРеквизитов.Свойство("ОжидаетсяВыборВариантаКП") Тогда
		Если ЗначенияРеквизитов.ОжидаетсяВыборВариантаКП Тогда
			ТекстОшибки = НСтр("ru='В документе %Документ% не выбран основной вариант коммерческого предложения.'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ДанныеЗаполнения);
			ВызватьИсключение ТекстОшибки;
		КонецЕсли;
	КонецЕсли;
	...
	Если ЗначенияРеквизитов.Свойство("СостояниеЗаказа") 
		//se 09/08/21 		
		И НЕ РольДоступна("саСнятиеРезерваПриЗавершенномЗаказе")
		//se 09/08/21
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	//Иначе
		ЗаголовокПредставление = НСтр("ru='Заявка на вывоз лома'");
	//КонецЕсли;
...
Процедура ПоляЗагрузкиДанныхИзВнешнегоИсточника(ТаблицаПолейЗагрузки, НастройкиЗагрузкиДанных) Экспорт
	...
	Если ПустаяСтрока(ПолноеИмяОбъектаЗаполнения) 
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СтруктурнаяЕдиницаРезерв", "Склад", ОписаниеТиповСтрока50, ОписаниеТиповКолонка, , , , , ВидимостьСклада);
	...
	ИначеЕсли ПолноеИмяОбъектаЗаполнения="Документ.ЗаказПокупателя.ТабличнаяЧасть.Калькуляция" Тогда
		...
		ЗагрузкаДанныхИзВнешнегоИсточника.ДобавитьПолеОписанияЗагрузки(ТаблицаПолейЗагрузки, "СебестоимостьЕдиницы", "Себестоимость ед.", ОписаниеТиповСтрока25, ОписаниеТиповЧисло15_2);
...
Процедура ПроверитьКорректностьДанныхВСтрокеТаблицы(СтрокаТаблицыФормы, ПолноеИмяОбъектаЗаполнения = "") Экспорт
	...
	Если ПустаяСтрока(ПолноеИмяОбъектаЗаполнения) 
		...
		СтрокаТаблицыФормы[ИмяСлужебногоПоля] = ЗначениеЗаполнено(СтрокаТаблицыФормы.Номенклатура)
			И НЕ СтрокаТаблицыФормы.Номенклатура.ЭтоНабор
...
Процедура СоздатьПоставляемыеШаблоныСообщений() Экспорт
	...
	Исключение
		...
		ОтменитьТранзакцию();
		ТекстОшибки = СтрШаблон(НСтр("ru='Не удалось заполнить справочник ""ШаблоныСообщений"" по умолчанию: %1'"), ОписаниеОшибки());
		ВызватьИсключение;
...
Процедура СформироватьПараметрыПоУслугам(Сообщение, ПредметСообщения)
	...
	Если ПредметСообщения.ВидОперации <> Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд Тогда
		Возврат;
	КонецЕсли;
...
Процедура ДобавитьКартинкуМастера(ФормируемыйПараметр, Движение, СписокРесурсов, ФотографииМастеров, Сообщение)
	...
	Если ЗначениеЗаполнено(КартинкаМастера[0].ФайлКартинки) Тогда
		СсылкаНаДвоичныеДанные = ПоместитьВоВременноеХранилище(УправлениеНебольшойФирмойСервер.ПолучитьДвоичныеДанныеФайла(КартинкаМастера[0].ФайлКартинки));
...
Функция ВыполненыУсловия(Предмет, Условия) Экспорт
	...
	ДанныеЗаказа = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Предмет, "СостояниеЗаказа,ВариантЗавершения,ВидЗаказа,ВидОперации");
...
Процедура АссистентУправленияОповеститьОтсутствиеОтгрузкиОплаты(Заказ, ЗначенияЗаполнения, ДополнительныеПараметры, Результат)
	...
	ИначеЕсли ДополнительныеПараметры.Событие = "ИзменениеСостоянияЗаказаПокупателя" Тогда
		...
		Если ЗаказВСостоянииНижеПроверяемого = Ложь Тогда
			...
			ВыполненоУсловиеОтгруженности = Истина;
			ВыполненоУсловиеОплаченности  = Истина;
			
			Если ЗначенияЗаполнения.Свойство("СтатусОтгрузки") И ЗначениеЗаполнено(ЗначенияЗаполнения.СтатусОтгрузки) Тогда
				ВыполненоУсловиеОтгруженности = Перечисления.СтатусОтгрузки.Сравнить(СтатусОтгрузки(Заказ), ЗначенияЗаполнения.СтатусОтгрузки) >= 0;
			КонецЕсли;
			
			Если ЗначенияЗаполнения.Свойство("СтатусОплаты") И ЗначениеЗаполнено(ЗначенияЗаполнения.СтатусОплаты) Тогда
				ВыполненоУсловиеОплаченности = Перечисления.СтатусОплаты.Сравнить(СтатусОплаты(Заказ), ЗначенияЗаполнения.СтатусОплаты) >= 0;
			КонецЕсли;
			
			Если НЕ ВыполненоУсловиеОтгруженности И НЕ ВыполненоУсловиеОплаченности Тогда
				Если ЭтоЗаказНаряд Тогда
				...
				КонецЕсли;
			ИначеЕсли НЕ ВыполненоУсловиеОтгруженности Тогда
				Если ЭтоЗаказНаряд Тогда
				...
				КонецЕсли;
			ИначеЕсли НЕ ВыполненоУсловиеОплаченности Тогда
	...
	Если ТекстСообщения <> Неопределено Тогда
		Сообщение = ОбсужденияСервер.НовыйДанныеСообщения();
...
Процедура СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя)
	...
	НомерЗапроса = 0;
	Запрос.Текст = УправлениеНебольшойФирмойСервер.ПолучитьТекстЗапросаКурсовыеРазницыРасчетыСПокупателями(Запрос.МенеджерВременныхТаблиц, Истина, НомерЗапроса, ПолучатьРеквизитыРасчетов);
...
Процедура СформироватьТаблицаОплатаСчетовИЗаказов(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	// НУЖНО СДЕЛАТЬ:
	// Проверить и удалить текст после строки "Возврат;".
	Возврат;
	...
	|	И ЗаказПокупателяПредоплата.ЭтоПредоплатаБезЗаказа
	|	И ЗаказПокупателяПредоплата.СуммаРасчетов <> 0
...
Процедура СформироватьТаблицаПродажиПоДисконтнойКарте(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	Если Выборка.Следующий() Тогда
		...
		Запрос.Текст = ДисконтныеКартыСервер.ТекстЗапросаПродажиПоДисконтнойКарте("ЗаказПокупателя");
...
Функция ПолучитьТекстЗапросаИнициализироватьДанныеДокументаЗаказНаряд()
	...
	|						КОГДА ЗаказНарядТовары.Номенклатура.ИспользоватьСерийныеНомера
	|							ТОГДА ЗаказНарядТовары.Количество
	|						ИНАЧЕ 1
	|					КОНЕЦ
	...
	|	ЗаказНарядТовары.Номенклатура.ИспользоватьСерийныеНомера КАК ИспользоватьСерийныеНомера,
	...
	|	ЗаказНарядМатериалы.КлючСвязиСерийныеНомера КАК КлючСвязиСерийныеНомера
	...
	|	ТаблицаДокумента.Документ.ИспользоватьСерийныеНомера КАК ИспользоватьСерийныеНомера,
	...
	|		ПО ЗаказПокупателяСерийныеНомера.КлючСвязи = ВременнаяТаблицаТовары.КлючСвязи
	|ГДЕ
	...
	|		ПО ЗаказПокупателяСерийныеНомера.КлючСвязи = ВременнаяТаблицаМатериалы.КлючСвязиСерийныеНомера
	|ГДЕ
	|	ЗаказПокупателяСерийныеНомера.Ссылка = &Ссылка
	|	И &ИспользоватьСерийныеНомера
...
Функция ПолучитьТекстЗапросаИнициализироватьДанныеДокументаЗаказПокупателя()
	...
	|	И ЗаказПокупателяКалькуляция.Ссылка.КалькуляцияРассчитана
	|	И ЗаказПокупателяКалькуляция.Ссылка.ОсновнойВариантКП = ЗаказПокупателяКалькуляция.НомерВариантаКП
	|	И ЗаказПокупателяКалькуляция.Себестоимость <> 0
	|
	|СГРУППИРОВАТЬ ПО
	|	ЗаказПокупателяКалькуляция.Ссылка.Дата,
	...
	|ГДЕ
	|	ЗаказНарядТовары.Ссылка = &Ссылка
	|	И ЗаказНарядТовары.Ссылка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийЗаказПокупателя.ЗаказНаПродажу)
	|	И ЗаказНарядТовары.Ссылка.ВариантЗавершения <> ЗНАЧЕНИЕ(Перечисление.ВариантыЗавершенияЗаказа.Отменен)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
...
Процедура ИнициализироватьДанныеДокументаЗаказНаряд(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя) 
	...
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст = ПолучитьТекстЗапросаИнициализироватьДанныеДокументаЗаказНаряд();
	...
	СформироватьТаблицаРасчетыСПокупателями(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя);
	...
	СформироватьТаблицаПродажиПоПредоставленнымСкидкам(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	// Эквайринг
	...
	СформироватьТаблицаПодарочныеСертификаты(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	СформироватьТаблицаОплатаПодарочнымиСертификатами(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства);
	...
	Если ПолучитьФункциональнуюОпцию("ПланироватьЗагрузкуРесурсовПредприятияРаботы") Тогда
		ТаблицаДвиженийРесурсы = ПланированиеРесурсовУНФ.ТаблицаДвиженийДокументов(ДокументСсылкаЗаказПокупателя);
...
Процедура ИнициализироватьДанныеДокументаЗаказПокупателя(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства, ДокументОбъектЗаказПокупателя) 
	...
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЭтапыПроизводства", МассивРезультатов[11].Выгрузить());
...
Процедура СформироватьТаблицаСуммыДокументовРегламентированныйУчет(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	Если ДокументСсылкаЗаказПокупателя.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд
...
Процедура СформироватьТаблицаПодарочныеСертификаты(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
	...
	Если СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьПодарочныеСертификаты Тогда
		...
		Если СтруктураДополнительныеСвойства.УчетнаяПолитика.ИспользоватьСерийныеНомера Тогда
			...
			|		ПО ВременнаяТаблицаТовары.КлючСвязи = СерийныеНомера.КлючСвязи
...
//
Процедура СформироватьТаблицаОплатаПодарочнымиСертификатами(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
...
Процедура СформироватьТаблицаПродажиПоПредоставленнымСкидкам(ДокументСсылкаЗаказПокупателя, СтруктураДополнительныеСвойства)
...
Процедура ВыполнитьКонтроль(ДокументОбъектЗаказПокупателя, ДополнительныеСвойства, Отказ, УдалениеПроведения = Ложь) Экспорт
	...
	Если СтруктураВременныеТаблицы.ДвиженияЗапасыНаСкладахИзменение
		...
		Если Не МассивРезультатов[7].Пустой() Тогда
			...
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибкахПроведенияПоРегиструПодарочныеСертификаты(ДокументОбъектЗаказПокупателя, ВыборкаИзРезультатаЗапроса, Отказ);
		КонецЕсли;
...
Процедура ПриПолученииЗначенийПараметровШаблона(ОбъектПечати, НазначениеШаблона, ОписаниеПараметров, ДополнительныеПараметры) Экспорт
	...
	Если НазначениеШаблона = Перечисления.НазначенияШаблоновПечатиОфисныхДокументов.КоммерческоеПредложение Тогда
		...
		КонтрагентЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ОбъектПечати, "Контрагент");
		ЗаполнитьЗначенияПараметровКонтрагента(КонтрагентЗаказа, ОписаниеПараметров);
...
Процедура ПриВыводеТаблицыПараметровШаблона(ПараметрыПечати, ИмяТаблицы, ОписаниеПараметров, ПараметрыВывода = Неопределено) Экспорт
	...
	|		Спецификация КАК Спецификация,
	|		СтранаПроисхождения КАК СерийныеНомера,
	|		СерийныеНомера КАК НомерГТД,
	|		Партия КАК Партия,
...
Процедура ДобавитьПараметрыЗаказаПокупателяВШаблон(ПараметрыШаблона)
	...
	ШаблоныПечатиОфисныхДокументов.ДобавитьПараметрТаблицыШаблона(ПараметрыТаблицыЗапасы, "ЗаказПокупателя.Запасы.СерийныеНомера", НСтр("ru='Серийные номера'"), ПрефиксТаблицаВладелец);
...
Процедура ЗаполнитьЗначенияПараметровКонтрагента(Контрагент, ОписаниеПараметров)
	...
	ШаблоныПечатиОфисныхДокументов.ЗаполнитьЗначенияКонтактнойИнформации(ОписаниеПараметров, "ЗаказПокупателя.Контрагент.", ДанныеКонтрагента.Ссылка);
...
Процедура ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиДокумента(СтрокаТабличнойЧасти, ОписаниеПараметров, СтруктураИтогов, ПрефиксПараметраСтроки, ПараметрыВывода)
	...
	Если ЭтоВыводВФайл Тогда
		...
		Если СтрокаТабличнойЧасти.ЭтоРазделитель Тогда
			...
			ДанныеПечати.Вставить(ПрефиксПараметраСтроки + "СерийныеНомера", "");
		...
		Иначе
			...
			ДанныеПечати.Вставить(ПрефиксПараметраСтроки + "СерийныеНомера", СтрокаТабличнойЧасти.СерийныеНомера);
...
Процедура ВывестиСекциюОбсуждение(Макет, Результат, Параметры)
	...
	|	Обсуждения.Пользователь КАК Пользователь,
	|	Обсуждения.Комментарий КАК Комментарий
	|ИЗ
...
Функция УниверсальныйЗапросПоДаннымДокумента(МассивОбъектов, ИспользоватьФаксимиле, Ошибки)
	...
	|	,ЗаказПокупателя.Ответственный.Физлицо КАК ФизическоеЛицоОтветсвенного
...
Функция ДанныеДокументовРегУчет(МассивОбъектов, ИспользоватьФаксимиле, ПечатнаяФормаТолькоВРублях = Истина, Ошибки = Неопределено) Экспорт
	...
	|		ЗаказПокупателя.Запасы.Количество КАК КоличествоМест,
	|		ВЫБОР
	|			КОГДА ЗаказПокупателя.Запасы.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	|					И ПечатьПользовательскихЕдиницИзмерения.Значение <> ЗНАЧЕНИЕ(Перечисление.ПечатьПользовательскихЕдиницИзмерения.ФормироватьПечатныйДокументВПользовательскойЕдинице)
...
Процедура СформироватьЗаказПокупателя(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		СтруктураИтогов = Новый Структура("Сумма, СуммаНДС, Всего, Количество, НомерСтроки, Вес, ЕстьСкидки, СкидкаПоСтроке, СкидкаПоДокументу, ПредставлениеСкидки, ЕстьСтавкаНольПроцентов", 0, 0, 0, 0, 0, 0, ЕстьСкидки, 0, 0, ПредставлениеСкидки, Ложь);
		ПараметрыНоменклатуры = Новый Структура;
		
		Для каждого СтрокаРаботыУслуги Из Шапка.ТаблицаРаботыУслуги Цикл
			
			ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиЗаказПокупателя(СтрокаРаботыУслуги, ДанныеПечати, ПараметрыНоменклатуры, СтруктураИтогов);
		...
		Для каждого СтрокаЗапасы Из Шапка.ТаблицаЗапасы Цикл
			...
			ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиЗаказПокупателя(СтрокаЗапасы, ДанныеПечати, ПараметрыНоменклатуры, СтруктураИтогов);
		...
		ДанныеПечати.Вставить("НДС", ПечатьДокументовУНФ.ПредставлениеЗаголовкаНДС(СтруктураИтогов.СуммаНДС, Шапка.СуммаВключаетНДС, Ложь, СтруктураИтогов.ЕстьСтавкаНольПроцентов));
		...
		ДанныеПечати.Вставить("ИтоговаяСтрока", ПечатьДокументовУНФ.ИтоговаяСтрока(СтруктураИтогов.Количество, СтруктураИтогов.Всего, Шапка.ВалютаДокумента));
...
Процедура СформироватьЗаказНаряд(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		ДанныеПечати = Новый Структура;
		ДанныеПечати.Вставить("ТекстЗаголовка", "Заказ-наряд № "
												+ НомерДокумента
												+ " от "
		...
		СтруктураИтогов = Новый Структура("НомерСтроки, СуммаРаботы, СуммаНДСРаботы, СуммаТовары, СуммаНДСТовары , Сумма, СуммаНДС, Всего, Количество, ЕстьСкидки, СкидкаПоДокументу, СкидкаПоСтроке, ПредставлениеСкидки", 0, 0, 0, 0, 0, 0, 0, 0, 0, ЕстьСкидки, 0, 0, ПредставлениеСкидки);
		...
		Для каждого СтрокаРаботы Из Шапка.ТаблицаРаботы Цикл
			...
			ОбластьМакета.Параметры.Заполнить(СтрокаРаботы);
 			...
 			ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиЗаказНаряд(СтрокаРаботы, ДанныеПечати, ПараметрыНоменклатуры, СтруктураИтогов);
			...
			Для каждого СтрИсполнитель Из СтрокиИсполнители Цикл
				ПредставлениеСотрудник = УправлениеНебольшойФирмойСервер.ПолучитьФамилиюИмяОтчество(СтрИсполнитель.Фамилия, СтрИсполнитель.Имя, СтрИсполнитель.Отчество);
				СтрокаИсполнители = СтрокаИсполнители + ?(ПустаяСтрока(СтрокаИсполнители), "", ", ") 
									+ ?(ЗначениеЗаполнено(ПредставлениеСотрудник), ПредставлениеСотрудник, СтрИсполнитель.Физлицо);
			КонецЦикла; 
			ДанныеПечати.Вставить("Исполнители", СтрокаИсполнители);
		...
		ОбластьМакета.Параметры.Всего = УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаРаботы);
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		Иначе
			...
			ОбластьМакета.Параметры.ВсегоНДС = УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаНДСРаботы);
		КонецЕсли; 
		ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		Если Шапка.ТаблицаЗапасы.Количество() > 0 Тогда
			...
			ОбластьМакета.Параметры.Всего = УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаТовары);
			ТабличныйДокумент.Вывести(ОбластьМакета);
			...
			Иначе
				ОбластьМакета.Параметры.НДС = ?(Шапка.СуммаВключаетНДС, "В том числе НДС:", "Сумма НДС:");
				ОбластьМакета.Параметры.ВсегоНДС = УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.СуммаНДСТовары);
			КонецЕсли; 
			ТабличныйДокумент.Вывести(ОбластьМакета);
		...
		Если Шапка.ТаблицаМатериалыЗаказчика.Количество() > 0 Тогда
			...
			Для каждого СтрокаМатериалыЗаказчика Из Шапка.ТаблицаМатериалыЗаказчика Цикл
				...
				ОбластьМакета.Параметры.Товар = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(СтрокаМатериалыЗаказчика.Товар, 
...
Процедура СформироватьПриложениеКДоговору(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	Перем ПервыйДокумент, НомерСтрокиНачало;
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		ЕстьСкидки = (Шапка.ТаблицаЗапасы.Итог("ЕстьСкидка") + Шапка.ТаблицаРаботы.Итог("ЕстьСкидка")) <> 0;
		
		СтруктураИтогов = Новый Структура("Сумма, СуммаНДС, Всего, Количество, НомерСтроки, ЕстьСкидки, СкидкаПоСтроке, СкидкаПоДокументу, ПредставлениеСкидки", 0, 0, 0, 0, 0, ЕстьСкидки, 0, 0, ПредставлениеСкидки);
		ПараметрыНоменклатуры = Новый Структура;
		...
		Для каждого СтрокаРаботы Из Шапка.ТаблицаРаботы Цикл
			...
			ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиПриложениеКДоговору(СтрокаРаботы, ДанныеПечати, ПараметрыНоменклатуры, СтруктураИтогов);
		...
		Для каждого СтрокаЗапасы Из Шапка.ТаблицаЗапасы Цикл
			...
			ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиПриложениеКДоговору(СтрокаЗапасы, ДанныеПечати, ПараметрыНоменклатуры, СтруктураИтогов);
		...
		ДанныеПечати.Очистить();
		ДанныеПечати.Вставить("Всего", УправлениеНебольшойФирмойСервер.ФорматСумм(СтруктураИтогов.Сумма));
		ДанныеПечати.Вставить("НДС", ПечатьДокументовУНФ.ПредставлениеЗаголовкаНДС(СтруктураИтогов.СуммаНДС, Шапка.СуммаВключаетНДС, Ложь));
...
Процедура СформироватьКалькуляцию(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати)
	...
	Если ТипЗнч(ОтображатьСебестоимость)<>Тип("Булево") Тогда
		ОтображатьСебестоимость = РольДоступна("ПолныеПрава") ИЛИ РольДоступна("ЧтениеДанныхУНФ");
	КонецЕсли; 
		...
		Пока ВыборкаИзделий.Следующий() Цикл
			...
			Пока ВыборкаСостава.Следующий() Цикл
				...
				Если ВыборкаСостава.ДополнительныйМатериал Тогда
					...
					ЗапасПредставление = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(
	...
	Если НЕ ОтображатьСебестоимость Тогда
		ТабличныйДокумент.УдалитьОбласть(ТабличныйДокумент.Область(, 23, , 28), ТипСмещенияТабличногоДокумента.ПоГоризонтали);
	КонецЕсли;
...
Процедура СформироватьКоммерческоеПредложение(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки)
	...
	Для каждого Шапка Из ДанныеДокументов Цикл
		...
		Пока НомерВарианта <= ПоследнийВариант Цикл
			...
			СтруктураИтогов = Новый Структура("Сумма, СуммаНДС, Всего, Количество, НомерСтроки, Вес, ЕстьСкидки", 0, 0, 0, 0, 0, 0, ЕстьСкидки);
			ПараметрыНоменклатуры = Новый Структура;
			...
			Для каждого СтрокаЗапасы Из Строки Цикл
				...
				ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиКоммерческоеПредложение(СтрокаЗапасы, ДанныеПечати, ПараметрыНоменклатуры, СтруктураИтогов);
				
				Если СтруктураИтогов.ЕстьСкидки Тогда
					ОбластьМакета = ?(ДанныеПечати.Скидка <> 0, ОбластьСтрокаСоСкидкойЗаполнено, ОбластьСтрокаСоСкидкойНеЗаполнено);
				Иначе
...
Функция ЗапросДанных(МассивОбъектов)
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	НАЧАЛОПЕРИОДА(ЗаявкаНаЛом.ДатаОтгрузки, ДЕНЬ) КАК Дата,
		|	ЗаявкаНаЛом.Номер КАК НомерЗаявкиВБазе,
		|	ВЫБОР
		|		КОГДА ЗаявкаНаЛом.СтруктурнаяЕдиницаРезерв = ЗНАЧЕНИЕ(Справочник.СтруктурныеЕдиницы.ПустаяСсылка)
		|			ТОГДА ЗаявкаНаЛом.Контрагент
		|		ИНАЧЕ ЗаявкаНаЛом.СтруктурнаяЕдиницаРезерв
		|	КОНЕЦ КАК Клиент,
		|	ЗаявкаНаЛом.ЗонаТариф КАК Направление,
		|	ЗаявкаНаЛом.ФИОВодителя КАК Водитель,
		|	ЗаявкаНаЛом.ОбъявленнаяЦенность КАК Заявленный,
		|	ЗаявкаНаЛом.Ссылка КАК Ссылка
		|ИЗ
		|	Документ.ЗаявкаНаЛом КАК ЗаявкаНаЛом
		|ГДЕ
		|	НЕ ЗаявкаНаЛом.ПометкаУдаления
		|	И ЗаявкаНаЛом.Ссылка В(&МассивОбъектов)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата,
		|	Водитель,
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)  Экспорт
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	Если ИмяМакета="ДляАгента" Тогда
		ТабличныйДокумент.КлючПараметровПечати = "ПараметрыПечати_ДляАгента";
		
		ПервыйДокумент = Истина;
		
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ЗаявкаНаЛом.ПФ_MXL_ДляАгента");
		
		ОбластьМакета = Макет.ПолучитьОбласть("ЗаголовокДокумента");
		ОбластьМакетаСтрока = Макет.ПолучитьОбласть("СтрокаДокумента");
		ОбластьМакетаПодвал = Макет.ПолучитьОбласть("ПодвалДокумента");
		...
		Пока ВыборкаДата.Следующий() Цикл
			...
			ОбластьМакета.Параметры.Дата =  ВыборкаДата.Дата;
			ТабличныйДокумент.Вывести(ОбластьМакета);             
	...
	//		
		//		
	//		НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
	//		НомерДокумента = " ЗЛ - "+ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ВыборкаДокументов.Номер, Истина, Истина);
	//		
	...
	//		
	//		СтруктураПараметров.Очистить();
	//		Если ЗначениеЗаполнено(ВыборкаДокументов.СлужбаДоставки) И НЕ ВыборкаДокументов.СлужбаДоставки = "Доставка собственными силами" Тогда
	...
	//			//Если 	НЕ ЗначениеЗаполнено(СтрокаЗаказ.ВремяС)
	...
	//		
	//		СтруктураПараметров.Очистить();
	//		Шаблон = НСтр("ru = 'Вес (всего):   %1 т.'");
	//		
	//		Шаблон1 = НСтр("ru = ' %1 т.'");
	//		
	//		СтруктураПараметров.Вставить("ДопИнформация", СтрШаблон(Шаблон, ИтогоВес/1000));
	//		СтруктураПараметров.Вставить("ДопИнформация1", СтрШаблон(Шаблон1,ИтогоВесЛома/1000));
	...
	//		
	//		НомерСтрокиНачало = ТабличныйДокумент.ВысотаТаблицы + 1;
	//		НомерДокумента = " МЛ - "+ПрефиксацияОбъектовКлиентСервер.НомерНаПечать(ВыборкаДокументов.Номер, Истина, Истина);
	//		
	//		ОбластьМакета = Макет.ПолучитьОбласть("ЗаголовокДокумента");
	//		СтруктураПараметров.Очистить();
	...
	//			ОбластьМакета.Параметры.Заполнить(СтруктураПараметров);
	//			
	//			 Если ТипЗнч(СтрокаЗаказ.Заказ) = Тип("ДокументСсылка.ЗаявкаНаЛом") И СтрокаЗаказ.Заказ.Запасы.Количество()>0 Тогда
	//				 УстановитьПривилегированныйРежим(Истина);
	//				 СкладLi = ПолучитьСкладLi();
	//			 	 Если ПривилегированныйРежим() Тогда
	//				         УстановитьПривилегированныйРежим(Ложь);
	//				 КонецЕсли; 
	//				 Если СкладLi =Неопределено Тогда
	//				          ОбластьМакета.Параметры.Примечание="";
	//					  Иначе
	//						    Если СтрокаЗаказ.Заказ.СтруктурнаяЕдиницаРезерв = СкладLi  Тогда
	//							         ОбластьМакета.Параметры.ТипОперации = Перечисления.ТипыОперацийМаршрута.Доставка;
	//									 ОбластьМакета.Параметры.Примечание="батарея из ремонта";
	//							ИначеЕсли 	СтрокаЗаказ.Заказ.СтруктурнаяЕдиницаПродажи = СкладLi  Тогда
	//							         ОбластьМакета.Параметры.ТипОперации = Перечисления.ТипыОперацийМаршрута.Забор;
	//									 ОбластьМакета.Параметры.Примечание="батарея в ремонт";
	//							КонецЕсли;
	//				 КонецЕсли; 
	//				 
	//			 Иначе
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	Перем Ошибки;
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ДляАгента") Тогда
		ТабДок =  ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, "ДляАгента");
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, "ДляАгента", "Печатная форма для агента", ТабДок);
	КонецЕсли;
	...
	//	ПечатнаяФорма.СинонимМакета = НСтр("ru ='Приложение к договору'");
	//	
	//	СформироватьПриложениеКДоговору(ПечатнаяФорма, МассивОбъектов, ОбъектыПечати, Ошибки);
	//	
	//КонецЕсли;
	//
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "ДляАгента";
	КомандаПечати.Представление = НСтр("ru = 'Печатная форма для агента'");
...
Процедура ЗаполнитьДанныеПечатиПоСтрокеТабличнойЧастиПриложениеКДоговору(СтрокаТабличнойЧасти, ДанныеПечати, ПараметрыНоменклатуры, СтруктураИтогов)
...
Функция ПолучитьАдресПогрузки(Склад)  Экспорт

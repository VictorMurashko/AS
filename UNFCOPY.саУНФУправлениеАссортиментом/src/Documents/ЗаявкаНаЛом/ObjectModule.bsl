...
Функция ЭтоЗаказНаряд() Экспорт
	...
	Возврат ЭтотОбъект.ВидОперации = Перечисления.ВидыОперацийЗаказПокупателя.ЗаказНаряд;
...
Процедура СформироватьПлатежнуюСсылку() Экспорт
	...
	СсылкаДляОплаты = ИнтеграцияСЯндексКассойСлужебный.ПлатежнаяСсылка(ВходящиеПараметры);
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	ЭтотОбъект.ВесФакт         =0;
	ЭтотОбъект.Доставлен     = 0;
	ЭтотОбъект.Вес              =0;
КонецПроцедуры


Функция ДатаДоставкиПоГрафику(СтруктурнаяЕдиницаРезерв,ДатаОбработки)
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	МИНИМУМ(НАЧАЛОПЕРИОДА(ГрафикиРаботы.ВремяНачала, ДЕНЬ)) КАК ВремяНачала
	|ИЗ
	|	РегистрСведений.ГрафикиРаботыМагазиновИФилиалов КАК ГрафикиРаботы
	|ГДЕ
	|	ГрафикиРаботы.ГрафикРаботы = &ГрафикРаботы
	|	И ГрафикиРаботы.Год = &Год
	|	И НАЧАЛОПЕРИОДА(ГрафикиРаботы.ВремяНачала, ДЕНЬ) > НАЧАЛОПЕРИОДА(&ДатаОбработки, ДЕНЬ)
	|	И КОНЕЦПЕРИОДА(ГрафикиРаботы.ВремяНачала, ДЕНЬ) <= КОНЕЦПЕРИОДА(ДОБАВИТЬКДАТЕ(&ДатаОбработки, МЕСЯЦ, 1), ДЕНЬ)";
	
	
	Запрос.УстановитьПараметр("ДатаОбработки",ДатаОбработки);
	Запрос.УстановитьПараметр("Год",Год(ДатаОбработки));
	Запрос.УстановитьПараметр("ГрафикРаботы",СтруктурнаяЕдиницаРезерв.саУНФ_ГрафикДоставки);

	
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
	         Возврат ДатаОбработки+3*86400;
		
		Иначе
	         Выборка =  РезультатЗапроса.Выбрать();
			 Выборка.Следующий();
			 Возврат Выборка.ВремяНачала;
	КонецЕсли; 
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ЭтотОбъект.ТипЗаявки = Перечисления.ТипыЗаявкиНаЛом.Лом;	
	Выборка=саУНФ_ОбщийМодуль.ПолучитьНастройкиЛогистики();
	СтруктураПоиска = Новый Структура("Параметр","Основной склад"); 
	Если Выборка.НайтиСледующий(СтруктураПоиска) Тогда
		СтруктурнаяЕдиницаПродажи = Выборка.Значение;
	КонецЕсли; 
    ДатаОтгрузки = ТекущаяДатаСеанса()+86400;
	ЗабратьУКлиента =Ложь;	
    ...
    ЭтотОбъект.ДокументОснование = ДанныеЗаполнения;
	 ...
	 Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПеремещениеЗапасов") Тогда
		 ЗабратьУКлиента =Ложь;
		 СтруктурнаяЕдиницаРезерв	=	ДанныеЗаполнения.СтруктурнаяЕдиницаПолучатель;
		 Если ЗначениеЗаполнено(СтруктурнаяЕдиницаРезерв) Тогда
			  ЭтотОбъект.ЗонаТариф	=	ЭтотОбъект.СтруктурнаяЕдиницаРезерв.НаправлениеДоставки;
			 АдресДоставки 					=  Документы.ЗаявкаНаЛом.ПолучитьАдресПогрузки(СтруктурнаяЕдиницаРезерв);
		 КонецЕсли; 
		 ДатаОтгрузки       =  ДанныеЗаполнения.ДатаОтгрузки;
		 ЭтотОбъект.ТипЗаявки = Перечисления.ТипыЗаявкиНаЛом.Лом;
	 КонецЕсли;
	 ...
	 Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриемИПередачаВРемонт") Тогда
		 ЭтотОбъект.ТипЗаявки = Перечисления.ТипыЗаявкиНаЛом.Литий;
		 ЗабратьУКлиента =Ложь;
		 //ЭтотОбъект.ДатаОтгрузки       =  ДанныеЗаполнения.Дата+3*86400;
		 
		 Если  ДанныеЗаполнения.РемонтВыполнен Тогда
			// ЭтотОбъект.ДатаОтгрузки       =  ДанныеЗаполнения.ДатаРемонтВыполнен +3*86400;
			 ЭтотОбъект.СтруктурнаяЕдиницаРезерв  = ДанныеЗаполнения.СкладОбмена;   //забрать из сервиса Li-ion
			 ЭтотОбъект.СтруктурнаяЕдиницаПродажи  =  ДанныеЗаполнения.СтруктурнаяЕдиница;
		 Иначе	
			 ЭтотОбъект.СтруктурнаяЕдиницаРезерв  		= 		ДанныеЗаполнения.СтруктурнаяЕдиница;   //забрать из РТТ
			 ЭтотОбъект.СтруктурнаяЕдиницаПродажи  	=   	ДанныеЗаполнения.СкладОбмена;
		 КонецЕсли;
		 
		 Если ЗначениеЗаполнено(ЭтотОбъект.СтруктурнаяЕдиницаРезерв) Тогда
			 ЭтотОбъект.ЗонаТариф							=	ЭтотОбъект.СтруктурнаяЕдиницаРезерв.НаправлениеДоставки;
			 ЭтотОбъект.АдресДоставки 					=  Документы.ЗаявкаНаЛом.ПолучитьАдресПогрузки(ЭтотОбъект.СтруктурнаяЕдиницаРезерв);
			 ЭтотОбъект.ДатаОтгрузки       				= 	ДатаДоставкиПоГрафику(ЭтотОбъект.СтруктурнаяЕдиницаРезерв,ДанныеЗаполнения.Дата);
		 КонецЕсли;

		 
		 
		  СтрокаТабЧасть= ЭтотОбъект.Запасы.Добавить();
		  СтрокаТабЧасть.Номенклатура 			=  ДанныеЗаполнения.Номенклатура;
		  СтрокаТабЧасть.СерийныеНомера 		=	ДанныеЗаполнения.СерийныйНомер;
		  СтрокаТабЧасть.Объем                   	=	50;
		  СтрокаТабЧасть.Количество            	=	1;
		  СтрокаТабЧасть.Вес                       		=	25;
		  СтрокаТабЧасть.ЕдиницаИзмерения	=	ДанныеЗаполнения.Номенклатура.ЕдиницаИзмерения;
	 КонецЕсли;
...
Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриемИПередачаВРемонт") Тогда
		  ЗаполнитьПоПриемуВРемонт(ДанныеЗаполнения);
	Иначе	
	         ЭтотОбъект.ТипЗаявки = Перечисления.ТипыЗаявкиНаЛом.Лом;
	КонецЕсли; 
	
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПриемИПередачаВРемонт") 
	ИЛИ 	 ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ПеремещениеЗапасов") 
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	КонецЕсли;
	 ...
	 Если ЗабратьУКлиента Тогда
	 
		 Если ПроверяемыеРеквизиты.Найти("СтруктурнаяЕдиницаРезерв") <> Неопределено Тогда
			 ПроверяемыеРеквизиты.Удалить(ПроверяемыеРеквизиты.Найти("СтруктурнаяЕдиницаРезерв"));
		 КонецЕсли;

	 
	 Иначе
	 
		 Если ПроверяемыеРеквизиты.Найти("Контрагент") <> Неопределено Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если  Не ЗначениеЗаполнено(ТипЗаявки) Тогда
	            ТипЗаявки = Перечисления.ТипыЗаявкиНаЛом.Лом;
	КонецЕсли; 
			
	Если ТипЗаявки=Перечисления.ТипыЗаявкиНаЛом.Литий И ЭтотОбъект.Запасы.Количество()=0  Тогда
		ОбщегоНазначения.СообщитьПользователю("Табличная часть пустая, не понятно что везти");
		Отказ = Истина;
	КонецЕсли;
...
Функция АдресДоставки(СсылкаНаСклад)
	АдрД = ""; 
	Запрос = Новый Запрос;
	Запрос.Текст = 
	...
	Запрос.УстановитьПараметр("Ссылка", СсылкаНаСклад);
	Результат = Запрос.Выполнить();
	
	Выборка = Результат.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		АдрД	=	Выборка.Представление;
	КонецЦикла;
...
Процедура ПриЗаписи(Отказ)
	...
	Если ТипЗаявки=Перечисления.ТипыЗаявкиНаЛом.Литий Тогда
	      ЗонаТариф	=	СтруктурнаяЕдиницаПродажи.НаправлениеДоставки;
		  АдресДоставки	= АдресДоставки(СтруктурнаяЕдиницаПродажи);
	ИначеЕсли ТипЗаявки=Перечисления.ТипыЗаявкиНаЛом.Лом Тогда
		  ЗонаТариф	=	СтруктурнаяЕдиницаРезерв.НаправлениеДоставки;
		  АдресДоставки	= АдресДоставки(СтруктурнаяЕдиницаРезерв);
	КонецЕсли; 

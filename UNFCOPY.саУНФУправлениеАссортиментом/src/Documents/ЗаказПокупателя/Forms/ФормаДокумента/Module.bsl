...
&НаСервере
Процедура саУНФ_УстановитьУсловноеОформлениеФормы()
	Если НЕ ТипЗнч("СписокКатегорий") = Тип("СписокЗначений") Тогда
	    СписокКатегорий=саУНФ_ОбщийМодуль.СписокКатегорий();
	КонецЕсли;
	    НовоеУсловноеОформление = УсловноеОформление.Элементы.Добавить();
	РаботаСФормой.ДобавитьЭлементОтбораКомпоновкиДанных(НовоеУсловноеОформление.Отбор,
														"Объект.Запасы.СерийныеНомера",
														"",
														ВидСравненияКомпоновкиДанных.Равно);

	
	РаботаСФормой.ДобавитьОформляемыеПоля(НовоеУсловноеОформление,
										  "ЗапасыСерийныеНомера");
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ТолькоПросмотр", Истина);
	РаботаСФормой.ДобавитьЭлементУсловногоОформления(НовоеУсловноеОформление, "ЦветТекста", WebЦвета.Синий);
КонецПроцедуры



&НаКлиенте
Процедура саУНФ_ЗапасыНачалоПеретаскиванияПеред(Элемент, ПараметрыПеретаскивания, Выполнение)
	ОбщегоНазначенияКлиент.СообщитьПользователю(Выполнение);
КонецПроцедуры
...
Процедура саУНФ_ПриСозданииНаСервереПосле2(Отказ, СтандартнаяОбработка)
	...
	Команда				= 	ЭтаФорма.Команды.Добавить("Гарантия");
	Команда.Действие	= 	"СоздатьГарантийныйТалон";
	Команда.Отображение	=	ОтображениеКнопки.КартинкаИТекст;
	Команда.Картинка	= 	БиблиотекаКартинок.Сегодня;
	...
	Если НЕ Параметры.Ключ.Пустая() Тогда
		...
		ДобавлениеКнопкиГарантия(Объект.Ссылка);
	КонецЕсли;
...
Процедура саУНФ_КонтрагентПриИзмененииПосле(Элемент)
    ...
    Ответ=ПроверитьНужныЛиГарантийныйТалонНаСервере();
	
	Если Ответ Тогда
		ЗаменитьКонтрагентаВГарантийнике();
	КонецЕсли;
...
#Область Гарантия
 &НаСервере
 Процедура ДоступностьКнопкиГарантия()
	ОМ = ОбщегоНазначения.ОбщийМодуль("саУНФ_ОбщийМодуль");
	Если ОМ = Неопределено Тогда
		Возврат;
	КонецЕсли;
	Если Не ЭтаФорма.Элементы.Найти("Гарантия") = Неопределено Тогда
		ДоступностьКнопки = ОМ.ВыписанныеГарантийныеТалоны(Объект.Ссылка);
		Элементы.Гарантия.Доступность = Не ДоступностьКнопки;
	КонецЕсли;
КонецПроцедуры
 
 &НаСервере
Процедура ДобавлениеКнопкиГарантия(СсылкаНаОбъект)
	ОМ = ОбщегоНазначения.ОбщийМодуль("саУНФ_ОбщийМодуль");
	Если ОМ = Неопределено Тогда
		Возврат;
	КонецЕсли;
	НужнаГарантия = ОМ.НужнаЛиГарантия(СсылкаНаОбъект);

	Если ЭтаФорма.Элементы.Найти("Гарантия") = Неопределено И НужнаГарантия Тогда
		Кнопка				= ЭтаФорма.Элементы.Добавить("Гарантия",
														   Тип("КнопкаФормы"),
														   ЭтаФорма.Элементы.ЗапасыКоманднаяПанельКоманды);
		Кнопка.Заголовок	= "Создать гарантийный талон";
		Кнопка.ИмяКоманды	= "Гарантия";
	
	    ДоступностьКнопкиГарантия();
	КонецЕсли;
КонецПроцедуры
...
Процедура саУНФ_ПослеЗаписиНаСервереПосле(ТекущийОбъект, ПараметрыЗаписи)
	ДобавлениеКнопкиГарантия(ТекущийОбъект.Ссылка);
КонецПроцедуры
 
 &НаКлиенте
Процедура СоздатьГарантийныйТалон()
	Ответ=ПроверитьНужныЛиГарантийныйТалонНаСервере();
	Если Ответ Тогда

		ТалоныНеВыписаны=ПроверитьВыписаныЛиГарантийки();
		Если НЕ ТалоныНеВыписаны Тогда
			ОбщегоНазначенияКлиент.СообщитьПользователю("Все гарантийные талоны выписаны");
			Возврат;
		Иначе
			ОписаниеОповещенияПослеСозданияГТ = Новый ОписаниеОповещения("ПослеВведенияГарантийногоТалона",ЭтаФорма);
			
			ПараметрыФормы = Новый Структура("Документ", Объект.Ссылка);

			ОткрытьФорму("ОбщаяФорма.ВводГарантийногоТалона",
						 ПараметрыФормы,
						 ЭтаФорма,
						 ЭтаФорма.УникальныйИдентификатор,
						 ,
						 ,
						 ОписаниеОповещенияПослеСозданияГТ,
						 РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
		КонецЕсли;

	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю("Гарантийные талоны не требуются");
	КонецЕсли;

КонецПроцедуры

&НаСервере
Функция ПроверитьВыписаныЛиГарантийки()
	ОМ = ОбщегоНазначения.ОбщийМодуль("саУНФ_ОбщийМодуль");
	Если ОМ = Неопределено Тогда
		Возврат Ложь;
	КонецЕсли;
	ВсеТалоныВыписаны = ОМ.ВыписанныеГарантийныеТалоны(Объект.Ссылка);
	Если ВсеТалоныВыписаны Тогда
		Возврат Ложь;
	КонецЕсли;
    Возврат Истина;
КонецФункции
...
&НаСервере
Функция ПроверитьНужныЛиГарантийныйТалонНаСервере()
	...
	НужнаГарантия = ОМ.НужнаЛиГарантия(Объект.Ссылка);
	Если НЕ НужнаГарантия Тогда
		Возврат Ложь;
	КонецЕсли;
	

	
	//МассивГарантийныхТалонов = ОМ.МассивГарантийныхТалоновНаВыписку(Объект.Ссылка);
	Возврат Истина;

КонецФункции

&НаСервере
Процедура ЗаменитьКонтрагентаВГарантийнике()
	ОМ = ОбщегоНазначения.ОбщийМодуль("саУНФ_ОбщийМодуль");
	Если ОМ = Неопределено Тогда
		Возврат;
	КонецЕсли;
	ОМ.ЗаменитьКонтрагентаВГарантийныхТалонах(Объект.Ссылка);

КонецПроцедуры

&НаКлиенте
Процедура ПослеВведенияГарантийногоТалона(Результат, Параметры) Экспорт
	Если НЕ Результат = Неопределено Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю("Гарантийные талоны созданы");
	КонецЕсли;
	ДоступностьКнопкиГарантия();
КонецПроцедуры
#КонецОбласти
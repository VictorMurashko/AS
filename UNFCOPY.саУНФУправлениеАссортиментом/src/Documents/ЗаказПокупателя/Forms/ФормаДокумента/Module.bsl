...
Процедура саУНФ_ПриСозданииНаСервереПосле2(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Параметры.Ключ.Пустая() Тогда
		...
		// Создадим кнопку и привяжем к ней команду
		Кнопка				= ЭтаФорма.Элементы.Добавить("Гарантия", Тип("КнопкаФормы"), ЭтаФорма.Элементы.ЗапасыКоманднаяПанельКоманды);
...
Процедура СоздатьГарантийныйТалон()
	...
	Если Не ТекСтрока = Неопределено Тогда
		СоздатьГарантийныйТалонНаСервере(ТекСтрока.Номенклатура);

		ПараметрыФормы = Новый Структура("Документ,Номенклатура", Объект.Ссылка,ТекСтрока.Номенклатура);

		ОткрытьФорму("ОбщаяФорма.ВводГарантийногоТалона", ПараметрыФормы);

	КонецЕсли;
КонецПроцедуры

&НаСервере
Процедура СоздатьГарантийныйТалонНаСервере(пНоменклатура)
	ОМ = ОбщегоНазначения.ОбщийМодуль("саУНФ_ОбщийМодуль");
	Если ОМ = Неопределено Тогда
		Возврат;
	КонецЕсли;	
	СсылкаНаОбъект = РеквизитФормыВЗначение("Объект");
	ОМ.НужнаЛиГарантия(СсылкаНаОбъект.Ссылка, пНоменклатура);
